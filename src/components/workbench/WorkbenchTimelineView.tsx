...
import type { InspectedObject } from "@/lib/workbench-types";
import {
  CalendarDays, AlertTriangle, ListChecks,
  CheckCircle2, Clock,
} from "lucide-react";
...
export function WorkbenchTimelineView({ matter, inspected, onInspect }: WorkbenchTimelineViewProps) {
          ...
          <div key={dateLabel} className="mb-6">
            ...
            <div className="ml-[7px] border-l border-transparent pl-6 space-y-1">
              {groupEntries.map((entry, ei) => {
                ...
                const isSelected =
                  inspected?.type === entry.type &&
                  inspected.index === entry.objectIndex &&
                ...
                return (
                  <div
                    key={`${dateLabel}-${ei}`}
                    onClick={() => onInspect({
                      type: entry.type as "interaction" | "commitment" | "signal",
                  ...
                  >
                    ...
                    {entry.status === "done" && <CheckCircle2 className="mt-0.5 h-3.5 w-3.5 text-[hsl(var(--success))]" />}
                    {entry.status === "overdue" && <Clock className="mt-0.5 h-3.5 w-3.5 text-[hsl(var(--critical))]" />}
                  </div>
                );
              })}
            </div>
